...
const Gallery = () => {
  ...
  const handleBackToSubAlbumClick = () => {
    console.log("Back to Sub-Album button clicked");
    if (selectedAlbum && selectedAlbum.parentAlbum) {
      console.log("Navigating to parent album:", selectedAlbum.parentAlbum);
  ...
  return (
    <div className="gallery-container" id="gallery">
      ...
      <Masonry columnsCount={2} gutter="10px" className="gallery">
        {albums[0].subAlbums[0].images.map((image, index) => (
          <img
            key={index}
            src={image}
            alt={`Image ${index + 1}`}
